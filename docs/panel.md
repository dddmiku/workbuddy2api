...
读账本时注意两点：账本只统计**经过本网关且已返回的原始用量**（同一台机器上直连其他服务商的流量不在内）；每轮重发上下文会再次产生输入量，其中的缓存命中已经包含在输入中，不能再相加一次。输出中的 reasoning_tokens 属于输出总数的明细，同样不重复相加。`server.input_token_scale` 只改变 Responses 客户端的上下文估计，不放大账本或计费数。缺少原始记录的历史无法从客户端估计或倍率反推出准确消耗。
